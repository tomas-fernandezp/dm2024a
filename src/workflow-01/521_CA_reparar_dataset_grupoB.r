...
Imputar_MICE <- function(dataset) {
  ...
  # Convertir data.table a data.frame para usar con mice
  data_df <- as.data.frame(dataset)
  
  
  # Reemplazar los valores 0 por NA en la columna cprestamos_personales
  data_df$ctrx_quarter[data_df$ctrx_quarter == 0] <- NA
  data_df$mcuentas_saldo[data_df$mcuentas_saldo == 0] <- NA
  data_df$active_quarter[data_df$active_quarter == 0] <- NA
  data_df$mactivos_margen[data_df$mactivos_margen == 0] <- NA
  data_df$ccomisiones_otras[data_df$ccomisiones_otras == 0] <- NA
  data_df$mtarjeta_visa_consumo[data_df$mtarjeta_visa_consumo == 0] <- NA
  data_df$mpasivos_margen[data_df$mpasivos_margen == 0] <- NA
  
  
  
  # Especificar las columnas a imputar
  impute_columns <- c("ctrx_quarter", "mcuentas_saldo", "active_quarter", "mactivos_margen", 
                      "ccomisiones_otras", "mtarjeta_visa_consumo", "mpasivos_margen")
  
  # Realizar la imputación usando MICE
  imputed_data <- mice(data_df[impute_columns], m = 5, method = 'pmm')
  
  
  # Si quieres ver el dataset imputado
  completed_data <- complete(imputed_data)
  
  # Reemplazar las columnas imputadas en el data.table original
  dataset[, (impute_columns) := completed_data]
  
  cat( "fin Imputar_MICE()\n")
}
